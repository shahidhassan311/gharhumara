                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="block">
                            ...
                            <div class="form-group">
                                @foreach ($purchase as $key => $items)
                                    <?php $main_id = $items->purchase_id  ; ?>
                                    <label class="col-md-3 control-label"></label>
                                    @foreach ($purchase1 as $key => $item)
                                        <img src="{{ URL::to('/') }}/uploads/{{ $item->images }}" height="200" width="200" alt="">
                                        <a href="/admin_purchase_images_delete/{{$item->id}}/{{ $main_id }}"><i class="fa fa-times fa-2x" aria-hidden="true" style="    position: absolute;margin-left: -21px;color: black;"></i></a>
                                    @endforeach
                                @endforeach
                            </div>
                            @foreach ($purchase as $key => $item)
                                <?php $purchase_id = $item->purchase_id ; ?>
                                <form action="{{url('/admin_purchase_store',$item->purchase_id)}}" role="form" class="form-horizontal" method="post" enctype="multipart/form-data" >
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-6">
                                            <input type="tetx"  value="{{ $item->amount }}" name="amount" class=" form-control">
                                    ...
                                    <a href="/user_purchase_request"><button class="btn btn-danger btn-group pull-right">Cancel</button></a>
    ...
    <script>
        ...
        xhttp1.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                document.getElementById("purchase_images").innerHTML = this.responseText;
            }
        };
        xhttp1.open("GET","/admin_purchase_display_images_ajax?purchase_id=<?php echo $purchase_id; ?>",true);
        xhttp1.send();
    </script>
@endsection
